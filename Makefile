...
ifeq ($(OS), Windows_NT)
	...
	ZIP_NAME = bin/palettizer_Win32$(TAG).zip
	...
	BIN_NAME = index.html
	LINKER_FLAGS += --embed-file $(ROMFILE) --shell-file $(WEB_SHELL) -s EXPORTED_FUNCTIONS='["_main"]' -s EXPORTED_RUNTIME_METHODS='["ccall","cwrap"]' -lidbfs.js
